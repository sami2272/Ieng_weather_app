...
import com.smart.weatherapp.remote.api.CitiesAPIService
...
import com.smart.weatherapp.remote.viewmodel.CitiesViewModel
...
import com.smart.weatherapp.ui.adapter.CityWeatherAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            locationViewModel.currentLocation.collectLatest {
                if (it.data?.size == 1) {
                    it.data[0].name?.let { it -> fetchweather(it) }
                    return@collectLatest
                }
                val fragment =
                    it.data?.let { cities ->
                        CityPickerFragment.newInstance(cities) { cities ->
                            lifecycleScope.launch {
                                cities.name?.let { it -> fetchweather(it) }
                            }
                        }
                    }
